...
export async function POST(req: Request) {
  try {
    const {
      ...
      selectedActivities,
      specialRequirements,
      user: userData, // Get the user data from the request body
    } = await req.json();
    ...
    const isNepalDestination = destination?.label?.toLowerCase().includes("nepal");
     ...
     // Base prompt
     let userPrompt = `
     Act as a professional travel planner (like a local guide in nepal). Your ONLY task is to create a detailed trip plan for the following destination: ${destination.label}.  Do not suggest any other destinations.
