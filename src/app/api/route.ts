...
function findNepalDestination(input: string) {
  ...
  for (const dest of nepalDatabase.destinations) {
    ...
    if (dest.nepaliName && normalizedInput.includes(dest.nepaliName.toLowerCase())) {
      return { name: dest.name, type: "main destination" };
    }
    ...
    if (dest.subRegions) {
      for (const subRegion of dest.subRegions) {
        if (normalizedInput.includes(subRegion.name.toLowerCase())) {
          return { name: `${subRegion.name}, ${dest.name}`, type: "sub-region" };
        }
      }
    }
...
export async function POST(req: Request) {
  try {
    ...
    const isNepalDestination = matchedDestination !== null || 
    ...
    let userPrompt = `
      ...
      CRITICAL INSTRUCTION: You MUST create a trip plan for EXACTLY this destination: "${userDestination}". 
      Do NOT substitute it with a more popular destination. Do NOT change the destination to a nearby city.
    ...
    if (matchedDestination) {
      ...
      I've identified that "${userDestination}" is in or near ${matchedDestination.name}. 
      You MUST focus the trip on "${userDestination}" specifically, while you can use information 
      about ${matchedDestination.name} to help with recommendations.
      `;
    }
    ...
    if (isNepalDestination) {
      ...
      If the user specified a small town, village, or local area in Nepal that's not in our database, please:
      1. Include it in the itinerary and make it the central focus
      2. You can reference the nearest major destination in our database for context
    ...
    userPrompt += `
      ...
      {
        "tripOverview": {
          "destination": "${userDestination}", // MUST be exactly this value, do not change it
          "duration": number,
        ...
        "hotels": [
          {
            "name": string,
            "location": string, // This should include ${userDestination} for at least one hotel
            ...
            "activities": [
              {
                "time": string,
                "activity": string,
                "location": string, // Most activities should be in ${userDestination}
                "duration": string,
                "cost": number,
                "notes": string
              }
        ...
        "restaurants": [
          {
            "name": string,
            "cuisine": string,
            "location": string, // At least one restaurant should be in ${userDestination}
            "priceRange": string,
      ...
      11. The destination MUST be "${userDestination}" exactly as provided by the user
    ...
    try {
      ...
      if (tripPlan.tripOverview && tripPlan.tripOverview.destination !== userDestination) {
        console.log(`Correcting destination from "${tripPlan.tripOverview.destination}" to "${userDestination}"`);
